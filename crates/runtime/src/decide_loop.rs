...
///
/// `query` 收到的 `StateQuery` 带上本次决策的 `decision_id`：宿主填的值被覆盖，
...
fn read_state(
    ...
    let query = query.clone().with_decision_id(decision_id);
...
///
/// `decision_id` 由调用方（`Router::route`）在进入前生成，这里只把它交给 state 的
...
pub fn run(
    ...
    seed: u64,    // 随机种子
    decision_id: &str,    // 本次决策 id，仅透传给 state.query
) -> Result<Decision, RouterError> {    // 返回的是 Result<Decision, RouterError> 类型。
    let (view, retrieved) = read_state(state, &req.routing_key(), hint, decision_id);
