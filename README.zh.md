...
反馈是具体结构：成功结果用 `Feedback::ok`，`route_id` 回填这次 `route` 返回的关联 id。`Overflow` / `Unavailable` 写在 `call.outcome` 上，会驱动排除 hint；`call = None` 表示结果未知。字段说明见 [`crates/protocol/README.md`](crates/protocol/README.md)。
...
零依赖底座。全部跨模块类型都在这里：`RouteRequest`、`Decision`、`ModelSelection`、`Feedback`、`StateView`、`RouterError`。其他 crate 只经协议层对话。类型与样例见 [`crates/protocol/README.md`](crates/protocol/README.md)。
